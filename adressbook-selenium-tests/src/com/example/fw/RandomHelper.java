...
public class RandomHelper extends HelperBase {
	...
	public String randomStringEngAlphaNumeric(final int length) {
		...
		if (rnd.nextInt(10) == 0)
			...
			return null;
		char[] chars = "abcdefghijklmnopqrstuvwxyz QWERTYUIOPASDFGHJKLZXCVBNM 1234567890 ._éöóêåíãøùçõúôûâàïğîëäæıÿ÷ñìèòüáş ÉÖÓÊÅÍÃØÙÇÕÚÔÛÂÀÏĞÎËÄÆİß×ÑÌÈÒÜÁŞ 1234567890 ._".toCharArray();
		StringBuilder sb = new StringBuilder();
		Random random = new Random();
		for (int i = 0; i < length; i++) {
		    char c = chars[random.nextInt(chars.length)];
	...
	public SortedListOf<Integer> randomIndexesList() {
	    ...
	    if (maxCount == 0) 
	    	throw new Error("Íåò ãğóïï äëÿ óäàëåíèÿ"); 
